...
func main() {
	...
	if err != nil {
		...
		fmt.Println("Run: bazel run //go/neuro/examples:mnist_cnn_full")
...
func startMockServer() {
	...
	m.HandleFunc("GET /demo", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/html")
		io.WriteString(w, "<h1>CNN Model Not Available</h1><p>Please train the CNN model first by running: bazel run //go/neuro/examples:mnist_cnn_full</p>")
	})
