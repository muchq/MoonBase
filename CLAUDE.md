...
- **Push and open the PR, then run the review panel** against that head — four
  independent agents on four distinct lenses, each refuting its own findings.
  Landing first makes the panel's work visible as commits on a green baseline
  rather than invisible inside the first diff. If it didn't run, say so rather
  than letting the reader assume it did.
