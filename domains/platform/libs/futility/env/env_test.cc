...
TEST(ReadPositiveInt, ReadsAWholeNumberOfSeconds) {
  setenv("FUTILITY_TEST_SECONDS", "30", 1);
  EXPECT_EQ(futility::env::ReadPositiveInt("FUTILITY_TEST_SECONDS"), 30);
  unsetenv("FUTILITY_TEST_SECONDS");
}

TEST(ReadPositiveInt, HasNoAnswerWhenItIsUnsetOrEmpty) {
  unsetenv("FUTILITY_TEST_SECONDS");
  EXPECT_EQ(futility::env::ReadPositiveInt("FUTILITY_TEST_SECONDS"), std::nullopt);
...
TEST(ReadPositiveInt, RefusesWhatItCannotRead) {
