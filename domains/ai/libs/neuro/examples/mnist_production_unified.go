...
func main() {
	...
	if *modelPath == "" {
		...
		if *modelPath == "" {
			...
			fmt.Println("  For dense network: bazel run //domains/ai/libs/neuro/examples:mnist_real")
			fmt.Println("  For CNN: bazel run //domains/ai/libs/neuro/examples:mnist_cnn_full")
			fmt.Println("\nOr specify a model path:")
			fmt.Println("  bazel run //domains/ai/libs/neuro/examples:mnist_production_unified -- -model=path/to/model")
			os.Exit(1)
		}
	}
