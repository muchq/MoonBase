...
func main() {
	...
	fmt.Println("Using embedded MNIST dataset from domains/ai/libs/neuro/data")
	fmt.Println()
	...
	fmt.Println("\nUsage: bazel run //domains/ai/libs/neuro/examples:mnist_cnn_full -- -train=5000 -test=1000 -epochs=20 -batch=64 -lr=0.001")
	...
	if err := inference.SaveModel(model.GetModel(), finalModelPath); err != nil {
		...
		fmt.Println("  bazel run //domains/ai/libs/neuro/examples:mnist_cnn_production")
	}
