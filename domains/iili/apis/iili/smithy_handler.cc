...
opal::Error Modeled(const char* code, std::string message) {
  opal::Error error = opal::Error::Modeled(code, message);
...
opal::Error ToSmithyError(const absl::Status& status) {
  ...
  return opal::Error::Unknown(std::move(message));
}

opal::Outcome<gen::ShortenOutput> SmithyShortenerHandler::Shorten(
    const gen::ShortenInput& input, [[maybe_unused]] const opal::server::RequestContext& context) {
...
opal::Outcome<gen::RedirectOutput> SmithyShortenerHandler::Redirect(
    const gen::RedirectInput& input, [[maybe_unused]] const opal::server::RequestContext& context) {
...
opal::Outcome<gen::RedirectHeadOutput> SmithyShortenerHandler::RedirectHead(
    const gen::RedirectHeadInput& input,
    [[maybe_unused]] const opal::server::RequestContext& context) {
  auto location = Location(input.slug);
  if (!location) return std::move(location).error();
  return gen::RedirectHeadOutput{.location = *std::move(location)};
}

opal::Outcome<std::string> SmithyShortenerHandler::Location(const std::string& slug) {
