...
class SmithyShortenerHandler final : public moonbase::iili::IiliHandler {
 public:
  ...
  opal::Outcome<moonbase::iili::ShortenOutput> Shorten(
  ...
  opal::Outcome<moonbase::iili::RedirectOutput> Redirect(
      const moonbase::iili::RedirectInput& input,
      const opal::server::RequestContext& context) override;

  /// Answers what Redirect answers; the transport does the framing.
  opal::Outcome<moonbase::iili::RedirectHeadOutput> RedirectHead(
 ...
 private:
  ...
  opal::Outcome<std::string> Location(const std::string& slug);
