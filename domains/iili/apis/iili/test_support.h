#ifndef DOMAINS_IILI_APIS_IILI_TEST_SUPPORT_H
#define DOMAINS_IILI_APIS_IILI_TEST_SUPPORT_H
...
inline std::shared_ptr<Shortener> MakeShortener(std::shared_ptr<UrlStore> store,
  ...
  return std::make_shared<Shortener>(
      ...
      std::make_shared<Shortener::Cache>(
          "url_cache", 100,
          std::make_shared<futility::otel::CapturingMetricsRecorder>("iili_test")),
      std::move(now));
}

}  // namespace iili

#endif  // DOMAINS_IILI_APIS_IILI_TEST_SUPPORT_H
