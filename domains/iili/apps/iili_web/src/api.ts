// The iili API (domains/iili/apis/iili), served at api.muchq.com behind
...
// Short links resolve on i.iili.uk (Caddy → iili). The SPA is on
...
export async function shorten(longUrl: string, expiresAt: number): Promise<{ slug: string }> {
  let res: Response;
  try {
    res = await fetch(`${API_BASE}/iili/v1/shorten`, {
      method: 'POST',
