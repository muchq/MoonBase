...
React + TypeScript frontend for the [iili](../../apis/iili) URL
...
SPA. A second frontend for the same API lives at muchq.com/iili (the
...
- **Redirect** — short links resolve on `i.iili.uk` (A-record to the
  ...
  `iili`'s `/iili/v1/r/{slug}` so the API's per-client rate limit still
...
- **Root directory:** `/domains/iili/apps/iili_web`
...
- The iili API deployed at `api.muchq.com`.
- Redirect host `i.iili.uk` on the consolidated Caddy (rewrite → iili).
