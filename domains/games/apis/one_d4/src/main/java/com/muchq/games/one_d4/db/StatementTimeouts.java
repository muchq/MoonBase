...
public final class StatementTimeouts {
   ...
   * the handle's statement config. The bound being part of the entry point — rather than a call
   * each statement must remember — is what makes "this path is bounded" structural: a new statement
   * either goes through here and is bounded, or visibly doesn't.
   *
   * <p>Nothing is unwound afterwards. pgjdbc scopes the timeout to the statement and enforces it
   * with a client-side cancel timer, so a bounded read returns its pooled connection carrying
   * neither a client field nor a server {@code statement_timeout} — pinned by {@code
   * PostgresReadTimeoutTest.boundDoesNotLeakAcrossStatementsOnPostgres}, which measures an
   * unbounded statement afterwards rather than reading a field that is 0 by construction.
   */
   ...
   * deletes anything so that a failing delete cannot undo it.
   */
