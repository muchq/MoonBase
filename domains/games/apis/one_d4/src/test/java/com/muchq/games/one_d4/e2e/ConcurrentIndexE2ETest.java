...
public class ConcurrentIndexE2ETest {
  ...
  public void upsertPeriod_failsWithMvccConflict_whenAnotherTransactionHoldsLock()
    ...
    // Arm latches for both the first attempt and the retry.
    ...
    latchPeriodStore.armUpsert(upsertReached, proceedWithUpsert, retryReached, proceedWithRetry);
    ...
    // Allow the first attempt — its MERGE blocks on our lock and times out with [90131].
    proceedWithUpsert.countDown();

    // Wait for Failsafe to retry and for the retry to reach its barrier in LatchIndexedPeriodDao.
    // At this point the worker is blocked before calling the delegate, so it is safe to release
    // our lock connection without any risk of it still being held when the delegate runs.
    retryReached.await();
    lockConn.rollback();
    lockConn.close();

    // Allow the retry to proceed — lock is now released, so the delegate succeeds.
    proceedWithRetry.countDown();

    String status = awaitTerminalStatus(10_000);
    assertThat(status)
        .describedAs("index request should succeed after retrying past the transient MVCC conflict")
        .isEqualTo("COMPLETED");
  }
