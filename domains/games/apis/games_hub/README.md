...
- `model/games.smithy` — `moonbase.games`: the service, session identity
  ...
  stats and the member's seat (`PlayerInfo.seat`: which table, pending or
  in play, absent while idle — how the lobby tells who is free, #1490).
  Apart from `GameSummary.game` and `Seat.game`, the word that names a
  table's game for the lobby, nothing here knows which game a table
  plays.
