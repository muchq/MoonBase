...
using games_hub::ChatRow;
using games_hub::PgChatStore;
...
TEST_F(PgChatStoreTest, AppendCommitsARowAndNotifiesItsChatChannel) {
  ...
  pg::Listener listener(
      ...
      /*on_active=*/nullptr);
  listener.Listen(games_hub::ChatChannel("R1"));
  ConfirmSubscribed(games_hub::ChatChannel("R1"), received);
...
TEST_F(PgChatStoreTest, PruningKeepsExactlyTheNewestHundred) {
  ...
  const auto recent = store_->LoadRecent("R1", games_hub::kChatHistoryLimit);
  ASSERT_TRUE(recent.ok()) << recent.status();
  ASSERT_EQ(recent->size(), games_hub::kChatHistoryLimit);
...
TEST_F(PgChatStoreTest, ConcurrentWritersOnSeparateConnectionsRetainTheNewestHundred) {
  ...
  const std::vector<int64_t> newest(
      committed.end() - static_cast<std::ptrdiff_t>(games_hub::kChatHistoryLimit), committed.end());

  const auto retained = store_->LoadRecent("R1", games_hub::kChatHistoryLimit);
