...
class TouchSpyStore final : public HubStore {
 public:
  ...
  /// Whether a sweep was enqueued within `budget`, and the first one's
  /// threshold.
  std::optional<std::chrono::seconds> AwaitSweep(std::chrono::milliseconds budget) {
...
TEST_F(GamesHubStreamFixture, CommandsOutsideARoomAreRejectedInBand) {
  ...
  ASSERT_TRUE(
      seat->stream.Send(GameCommands::FromGetroomstate(moonbase::games::GetRoomState{})).ok());
  ...
  moonbase::games::JoinRoom join;
  join.roomId = "r-nope";
  ASSERT_TRUE(seat->stream.Send(GameCommands::FromJoinroom(join)).ok());
  ...
  ASSERT_TRUE(seat->stream.Send(GameCommands::FromJoinroom(nul_join)).ok());
  ...
  ASSERT_TRUE(seat->stream.Send(GameCommands::FromCreateroom(moonbase::games::CreateRoom{})).ok());
...
TEST_F(GolfGameFixture, ChatReachesTheRoom) {
  ...
  chat.text = "good luck!";
  ASSERT_TRUE(table->alice.stream.Send(GameCommands::FromChat(chat)).ok());
  ...
  ASSERT_TRUE(table->alice.stream.Send(GameCommands::FromChat(second)).ok());
  ...
  ASSERT_TRUE(table->alice.stream.Send(GameCommands::FromChat(empty)).ok());
  ...
  blank.text = "   \t\n";
  ASSERT_TRUE(table->alice.stream.Send(GameCommands::FromChat(blank)).ok());
  EXPECT_TRUE(ReceiveCase(table->alice.stream, "commandRejected").has_value());

  moonbase::games::Chat oversized;
  oversized.text = std::string(501, 'x');
  ASSERT_TRUE(table->alice.stream.Send(GameCommands::FromChat(oversized)).ok());
  ...
  mangled.text = "hi\xC3";
  ASSERT_TRUE(table->alice.stream.Send(GameCommands::FromChat(mangled)).ok());
...
TEST_F(GamesHubStreamFixture, JoiningReplaysChatHistoryAfterRoomState) {
  ...
  live.text = "welcome";
  ASSERT_TRUE(alice->stream.Send(GameCommands::FromChat(live)).ok());
...
TEST_F(GamesHubStreamFixture, ShareLinkJoinSucceedsOnceBootGraceClearsStaleMembership) {
  ...
  ASSERT_TRUE(dave->stream.Send(GameCommands::FromJoinroom(join_old)).ok());
  ...
  ASSERT_TRUE(dave_back->stream.Send(GameCommands::FromJoinroom(join_shared)).ok());
  ...
  ASSERT_TRUE(alice_back->stream.Send(GameCommands::FromJoinroom(join_shared)).ok());
...
TEST_F(GamesHubStreamFixture, BuildingAHandlerDeclaresEveryCounterSeriesAtZero) {
  ...
  EXPECT_EQ(GolfHub::DeclaredCounterSeries().size(), kExpected.size());
...
// The hub_*/golf_* command and event declarations are a hand copy of the
// model's union cases, and this test is what makes a hand copy safe to keep:
// it reads the model and fails on drift in either direction — a case added to
// the model without a declaration loses its first event after every deploy
// (#1323), and a declaration for a case the model no longer has is a
// permanently flat line on a dashboard. CountCommand and Send name series
// exactly this way: the room layer's outer case on hub_*, each envelope's
// inner case on that tenant's series.
TEST(StreamSeriesModelPin, StreamSeriesMatchTheModelUnions) {
  ...
  EXPECT_EQ(DeclaredLabelValues("golf_commands", "command"),
            std::set<std::string>(moves.begin(), moves.end()));
  EXPECT_EQ(DeclaredLabelValues("golf_events", "event"),
            std::set<std::string>(updates.begin(), updates.end()));
}

// The lobby envelope (#1490) counts its inner case names on lobby_commands
// and lobby_events, pinned against lobby.smithy the same way.
TEST(StreamSeriesModelPin, LobbySeriesMatchTheModelUnions) {
  const std::string model = ReadModel("domains/games/apis/games_hub/model/lobby.smithy");
...
TEST_F(GamesHubStreamFixture, RejectionsCountTheBoundedKindNeverTheReason) {
  ...
  ASSERT_TRUE(
      table->alice.stream.Send(GameCommands::FromCreateroom(moonbase::games::CreateRoom{})).ok());
  ...
  EXPECT_EQ(metrics_->CounterTotal("hub_rejections", {{"kind", "rate_limited"}}), 0);
  EXPECT_EQ(metrics_->CounterTotal("hub_rejections", {{"kind", "unavailable"}}), 0);
  EXPECT_EQ(metrics_->CounterTotal("hub_rejections", {{"kind", "unknown"}}), 0);
...
TEST_F(GamesHubStreamFixture, AStoreOutageOnTheJoinPathsCountsAsUnavailable) {
  ...
  EXPECT_EQ(capture->CounterTotal("hub_rejections", {{"kind", "unavailable"}}), 2);
...
TEST_F(GamesHubStreamFixture, AnUnreachableStoreCountsTheAppendAsUnavailable) {
  ...
  EXPECT_EQ(capture->CounterTotal("hub_rejections", {{"kind", "unavailable"}}), 1);
}
...
TEST_F(GamesHubStreamFixture, AStaleMembershipCountsTheAppendAsRejected) {
  ...
  EXPECT_EQ(capture->CounterTotal("chat_appends", {{"result", "stored"}}), 0);
  EXPECT_EQ(capture->CounterTotal("hub_rejections", {{"kind", "state"}}), 1);
}
...
TEST_F(RateLimitedStreamFixture, AChatFloodStoresTheBurstAndRejectsTheRest) {
  ...
  EXPECT_EQ(metrics_->CounterTotal("hub_rate_limited", {{"kind", "chat"}}), 1);
