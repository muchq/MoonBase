...
public class QueryGamesTool implements McpTool {
  ...
  public String getDescription() {
    return "Search indexed games using ChessQL (index first with index_chess_games). Example"
        ...
        + " (equality only, 'month = \"2026-07\"'). date and month filter the indexed corpus"
        + " only, so a period that was never indexed comes back with zero games rather than an"
        + " error — indistinguishable from 'played no games then' unless you index that period"
        + " first with index_chess_games. Note: opening.family is derived from"
