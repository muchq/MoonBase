...
describe('CopyButton', () => {
  it('copies the exact text and confirms', async () => {
    ...
    render(<CopyButton text="https://r3dr.net/r/AQA" />);

    fireEvent.click(screen.getByRole('button', { name: 'Copy https://r3dr.net/r/AQA' }));

    expect(await screen.findByRole('button', { name: 'Copied' })).toHaveTextContent('Copied ✓');
    expect(writeText).toHaveBeenCalledWith('https://r3dr.net/r/AQA');
  });
