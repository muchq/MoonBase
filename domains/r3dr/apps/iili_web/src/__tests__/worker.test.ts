...
describe('worker', () => {
  it('302s GET /r/{slug} to the v2 API redirect route', async () => {
    const env = makeEnv();
    const response = await worker.fetch(new Request('https://iili.uk/r/AQA'), env);
  ...
  it('gives HEAD the same answer as GET — unfurlers and probes see the redirect', async () => {
    ...
    const response = await worker.fetch(
      new Request('https://iili.uk/r/AQA', { method: 'HEAD' }),
      env
    );
    expect(response.status).toBe(302);
    expect(env.ASSETS.fetch).not.toHaveBeenCalled();
  ...
  it('serves everything else from assets', async () => {
    ...
    for (const url of [
      ...
      'https://iili.uk/r/', // no slug
      'https://iili.uk/r/AQA/extra', // not a slug path
    ]) {
  ...
  it('leaves non-GET /r/ requests to the SPA fallback, not the API', async () => {
    const env = makeEnv();
    await worker.fetch(new Request('https://iili.uk/r/AQA', { method: 'POST' }), env);
