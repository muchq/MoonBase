...
public final class ImageUtils {
  ...
  public static BufferedImage convolveAndScaleByKernelSum(BufferedImage input, int[] kernel) {
    ...
    int[] convOutput =
        convolve(inputPixels, input.getWidth(), input.getHeight(), kernel, (i) -> i / kernelSum);
   ...
   *     pixels in the 2d matrix adjacent in the 1d array. To find the pixel immediately below a
   *     pixel at index i, look at index i + width
  ...
  public static int[] convolve(
      int[] input, int width, int height, int[] kernel, Function<Integer, Integer> scaler) {
   ...
   */
  public static int[] convolve(
      byte[] input, int width, int height, int[] kernel, Function<Integer, Integer> scaler) {
    // TODO: assert that kernel.length is an odd square (3x3, 5x5, 7x7)
    // TODO: what is a GPU?
    final int edgeOffset = (int) Math.sqrt(kernel.length) / 2;
    final int[] outputPixels = new int[input.length];
