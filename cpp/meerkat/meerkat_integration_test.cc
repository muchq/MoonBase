...
TEST_F(MeerkatIntegrationTest, JsonPostRequest) {
  ...
  auto response = client_->post_json(GetBaseUrl() + "/api/users", request_data, 10000);  // 10s timeout
...
TEST_F(MeerkatIntegrationTest, MiddlewareAuthentication) {
  ...
  std::cout << "First request completed: " << response_no_auth.success << ", " << response_no_auth.status_code << std::endl;
  ...
  std::cout << "Second request completed: " << response_with_auth.success << ", " << response_with_auth.status_code << std::endl;
...
TEST_F(MeerkatIntegrationTest, QueryParameters) {
  ...
  std::cout << "Response success: " << response.success << ", status: " << response.status_code << std::endl;
