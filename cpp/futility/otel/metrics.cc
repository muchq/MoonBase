...
void MetricsRecorder::RecordCounter(const std::string& metric_name, 
                                   int64_t value,
  ...
  if (it->second) {
    ...
    if (attributes.empty()) {
      ...
      auto kv_iterable = opentelemetry::common::KeyValueIterableView<std::vector<std::pair<std::string, std::string>>>(attr_vec);
...
void MetricsRecorder::RecordGauge(const std::string& metric_name,
                                 double value,
                                 const std::map<std::string, std::string>& attributes) {
  if (!meter_) return;
