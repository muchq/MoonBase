...
public class SqlCompiler implements QueryCompiler<CompiledQuery> {
    ...
    private static final Set<String> VALID_MOTIFS = Set.of(
            "pin", "cross_pin", "fork", "skewer", "discovered_attack"
    );
    ...
    private String compileExpr(Expr expr, List<Object> params) {
        return switch (expr) {
            case OrExpr or -> or.operands().stream()
                    .map(e -> compileExpr(e, params))
                    .collect(Collectors.joining(" OR ", "(", ")"));
            case AndExpr and -> and.operands().stream()
                    .map(e -> compileExpr(e, params))
    ...
    private String compileIn(InExpr in, List<Object> params) {
        ...
        String placeholders = in.values().stream()
                .map(v -> "?")
