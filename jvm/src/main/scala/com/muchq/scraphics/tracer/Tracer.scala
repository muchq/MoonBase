...
object Tracer {
  ...
  private def canvasToViewport(canvasPoint: Vec2, image: Image, scene: Scene): Vec3 =
    Vec3(
      canvasPoint._1 * scene.viewportSize / image.width,
      canvasPoint._2 * scene.viewportSize / image.height,
  ...
  private def intersectRaySphere(origin: Vec3, direction: Vec3, sphere: Sphere): (Double, Double) =
    ...
    if discriminant < 0 then return (INF, INF)
  ...
  private def specularLightIntensity(
      normal: Vec3,
      ray: Vec3,
      view: Vec3,
      light: Light,
  ...
  private def computeLighting(
    ...
    do
      if light.lightType == LightType.AMBIENT then intensity += light.intensity
      else
          ...
          if specular > 0 then
            ...
            if rDotV > 0 then
              intensity += (light.intensity * Math.pow(
                rDotV / (reflectedRay.length() * view.length()),
                specular
              ))
  ...
  private def traceRay(
    ...
    val (closestSphereMaybe, closestT) =
    ...
    closestSphereMaybe match
      ...
      case Some(s) =>
        ...
        if recursionDepth <= 0 || s.reflective <= 0 then return localColor

        val reflectedRay: Vec3 = reflectRay(normal, view)
        val reflectedColor: Color =
          traceRay(point, reflectedRay, EPSILON, INF, scene, recursionDepth - 1)
