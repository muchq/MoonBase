...
public class ServiceTest {
  ...
  private static final HttpClient client = new ApacheHttpClient();
  ...
  public void itReturns404OnEmptyOptionals() {
    HttpRequest request = HttpRequest.newBuilder().setUrl(baseUrl + "/test/optional-empty").build();
    HttpResponse response = client.execute(request);
  ...
  public void itReturns404OnNotFound() {
    HttpRequest request = HttpRequest.newBuilder().setUrl(baseUrl + "/test/not-found").build();
    HttpResponse response = client.execute(request);
    assertThat(response.getStatusCode()).isEqualTo(404);
  }
  ...
  public void itReturns500OnServerError() {
    HttpRequest request = HttpRequest.newBuilder().setUrl(baseUrl + "/test/server-error").build();
    HttpResponse response = client.execute(request);
  ...
  public void itReturns400OnBadRequest() {
    HttpRequest request = HttpRequest.newBuilder().setUrl(baseUrl + "/test/bad-request").build();
    HttpResponse response = client.execute(request);
